...
export async function getStudentMonitoring() {
    ...
    return students.map(student => {
        ...
        return {
            ...
            halaqah: (Array.isArray(student.halaqah) ? student.halaqah[0]?.name : student.halaqah?.name) || "-",
            shift: (Array.isArray(student.shift) ? student.shift[0]?.name : student.shift?.name) || "-",
            totalSPP,
            lastSPPDate,
            totalKas,
            saldoTabungan,
