...
import { Loader2, Pencil, Trash2, Filter, X, Trash, Search } from "lucide-react"
...
import { getFilteredTransactionsKomite, updateTransactionKomite, deleteTransactionKomite, bulkDeleteTransactionsKomite } from "@/actions/komite-reports"
...
export default function KomiteReportsPage() {
    ...
    }, [currentPage, startDate, endDate, categoryFilter, typeFilter, searchQuery])
    ...
    async function loadTransactions() {
        try {
            setIsLoading(true)
            const result = await getFilteredTransactionsKomite({
    ...
    async function onEditSubmit(values: z.infer<typeof editFormSchema>) {
        try {
            await updateTransactionKomite(editingTransaction.id, {
                type: values.type,
            ...
            console.error(error)
            showToast(error.message || "Gagal memperbarui transaksi", "error")
        }
    }
    ...
    async function handleDelete(id: string) {
        ...
        try {
            await deleteTransactionKomite(id)
    ...
    async function handleBulkDelete() {
        ...
        try {
            await bulkDeleteTransactionsKomite(Array.from(selectedIds))
